...
class MarstekUDPClient:
    ...
    def __init__(self, hass, host: str | None = None, port: int = DEFAULT_UDP_PORT, remote_port: int | None = None) -> None:
        ...
        Args:
            ...
            remote_port: Remote port to send to (defaults to DEFAULT_UDP_PORT)
        """
        self.hass = hass
        self.host = host
        self.port = port
    ...
    def _record_command_result(
        ...
        *,
        ...
        if error_code == ERROR_METHOD_NOT_FOUND:
            ...
            if stats["unsupported_error_count"] >= 3:
    ...
    def _get_broadcast_addresses(self) -> list[str]:
        """Get all broadcast addresses for local subnets."""
        broadcast_addrs = set()
        ...
        try:
            ...
            # Fallback: Use socket to get local IP and compute broadcast
            try:
                ...
                # Simple approach: assume /24 subnet and compute broadcast
                ip_parts = local_ip.split(".")
                if len(ip_parts) == 4:
                    broadcast = f"{ip_parts[0]}.{ip_parts[1]}.{ip_parts[2]}.255"
                    broadcast_addrs.add(broadcast)
            except Exception as err:
                _LOGGER.warning("Could not determine local IP for broadcast: %s", err)
        except Exception as err:
            _LOGGER.warning("Could not determine broadcast addresses: %s", err)

        # Always include the general broadcast as fallback
    ...
    async def get_energy_system_mode(self, instance_id: int = 0) -> dict[str, Any]:
        ...
        return await self.send_command(METHOD_ES_MODE, {"id": instance_id})
    ...
    async def set_energy_system_mode(self, instance_id: int, mode_config: dict[str, Any]) -> dict[str, Any]:
        ...
        return await self.send_command(METHOD_ES_SET_MODE, params)

    async def get_energy_meter_status(self, instance_id: int = 0) -> dict[str, Any]:
        """Get energy meter status."""
        return await self.send_command(METHOD_EM_STATUS, {"id": instance_id})
    ...
    async def get_all_data(self) -> dict[str, Any]:
        ...
        try:
            ...
            data["energy_system"] = await self.get_es_status()
